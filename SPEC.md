...
### Etapa 6 — Solicitudes de Membresía y Portal Público ✅ (2026-08-28)
- [x] Formulario público + creación de cuenta No Socio — `POST /api/solicitudes-membresia` (`[AllowAnonymous]`), primer endpoint sin autenticación de todo el sistema. Verificado en vivo de punta a punta: alta anónima por curl → login inmediato con la contraseña elegida en el form → unicidad cruzada de DNI probada (rechaza contra un `Socio` existente y contra otra solicitud `Pendiente`). Adjuntos (documento/ficha médica) vía `POST .../adjuntos` (también anónimo, identificado por Id no adivinable — el solicitante todavía no tiene sesión en ese punto del flujo).
- [x] Gestión administrativa de solicitudes + conversión a Socio (RF-SOL-13) — verificado en vivo: Empleado puede leer y anotar observaciones (`PUT /api/solicitudes-membresia/{id}`) pero no tiene los permisos `.aprobar`/`.rechazar` (matriz §2.2, nota al pie — acción irreversible con impacto en facturación); SuperAdmin aprobó una solicitud real y se creó un `Socio` (`S000003`) con categoría default (sin `CategoriaPretendidaId`, cayó a la primera Categoría Activa) y el `ApplicationUser` existente pasó de rol `NoSocio` a `Socio` sin duplicar la cuenta — confirmado con un segundo login que el rol cambió.
- [x] Seguimiento de estado por el solicitante — `GET /api/solicitudes-membresia/{id}/seguimiento`, verificado en vivo con la cuenta recién creada, con la validación de pertenencia (un No Socio no puede ver el seguimiento de otro).

**Extensión de `ConfiguracionGeneral` (tercera vez, Etapas 3/5/6)**: datos institucionales del club (`NombreClub`, `Cuit`, `Direccion`, `Telefono`, `EmailContacto`, `HorariosFuncionamiento`), expuestos también sin autenticar vía `GET /api/configuracion/publica` (subconjunto — nunca se expone la configuración financiera/de acceso sin login). La landing `/` (placeholder desde Etapa 0) se completó con estos datos + CTA a la solicitud — verificado en vivo.

**Bug real encontrado y corregido durante la implementación** (antes de la reconciliación, por el propio agente de continuación): los 4 permisos nuevos (`solicitudes-membresia.leer/editar/aprobar/rechazar`) estaban sembrados en la tabla `Permisos` pero nunca asignados a ningún Rol — nadie habría podido usar el módulo en la práctica.

**Nota de reconciliación:** ambos agentes se quedaron sin sesión a mitad de camino (el de frontend, además, delegó a un sub-agente en vez de hacer el trabajo directamente — se verificó que ese sub-agente sí estaba corriendo de verdad antes de esperar su resultado). Retomados como continuaciones sin perder el trabajo ya hecho. Encontré y corregí 3 mismatches de contrato reales: `PUT /api/solicitudes-membresia/{id}/observaciones` no existe como sub-ruta — es el mismo `PUT /{id}` de edición general; `POST .../rechazar` espera `{ motivoRechazo }`, no `{ motivo }`; `POST .../aprobar` no acepta ningún body (el frontend tenía un selector de categoría que el backend nunca iba a leer, ya que resuelve la categoría default por su cuenta — se quitó esa lógica del diálogo de aprobación).
...
- [x] Pruebas de carga sobre generación batch de cuotas y webhook de Mercado Pago ✅ (2026-08-29) — enfoque acordado con el usuario: test cronometrado .NET para el batch (mide escalabilidad de datos, sin herramientas nuevas) + k6 para el webhook (mide concurrencia real, corre en Docker). Medido contra el stack real: batch de 5.000 Socios activos → **3,46 s** (umbral de partida 15 s, sin datos residuales al terminar, verificado con `SELECT COUNT(*)`). Webhook con 20 VUs sostenidos ~30 s → 84.262 requests, 100% éxito, ~2.107 req/s, p95 = 14,68 ms. **Limitación documentada, no oculta**: sin credenciales reales de Mercado Pago en este entorno, la prueba del webhook mide el camino de parseo + salida temprana (`PagosController.Webhook` corta antes de llegar a la cascada de escritura a Cuota/Reserva cuando `MercadoPago:AccessToken` está vacío) — no la confirmación de pago real. Detalle completo en `backend/loadtests/README.md`.
- [x] Revisión de seguridad (OWASP Top 10) sobre endpoints públicos (solicitud de membresía, login) ✅ (2026-08-29) — el SPEC solo mencionaba 2 endpoints; mapeé los 7 `[AllowAnonymous]` reales (login, forgot/reset-password, solicitud de membresía + adjuntos, webhook de Mercado Pago, configuración pública) y acordé con el usuario 5 fixes concretos, grounded en el código (no una checklist genérica de OWASP): (1) **rate limiting** nativo de .NET 8 (`FixedWindowLimiter`, 3 policies: `auth` 10/min, `solicitud-membresia-publica` 5/min, `webhook-mp` 60/min — valores de partida ajustables, no SLA) + **lockout real de login** (`UserManager.AccessFailedAsync`/`IsLockedOutAsync`, 5 intentos → 15 min de bloqueo, mismo mensaje genérico "Credenciales inválidas." para no crear un oráculo de enumeración); (2) validación de tipo/tamaño de archivo en adjuntos (`ArchivoAdjuntoValidator`: whitelist PDF/JPG/PNG, 5MB por archivo, antes solo había límite de request completo); (3) corregido un bug real en `PagosController.Webhook` — la validación de firma HMAC dependía de si `WebhookSecret` estaba seteado, independiente de si Mercado Pago estaba configurado (`AccessToken`) — si en producción se cargaba `AccessToken` sin `WebhookSecret`, el webhook procesaba pagos sin validar firma; ahora falla con 500 en vez de saltear la validación; (4) actualizado `Microsoft.AspNetCore.DataProtection` (y paquetes de la misma familia) de 8.0.19 a 8.0.30, resolviendo 8 advisories High de `System.Security.Cryptography.Xml` (dependencia transitiva); (5) `MaximumLength` agregado a los campos de `Login` y a 5 campos opcionales de `CrearSolicitudMembresiaRequest` sin ninguna validación de longitud. Verificado: 78/78 tests (32 nuevos, incluyendo el primer test del repo con `WebApplicationFactory<Program>`), `dotnet list package --vulnerable` en 0 advisories sobre los 4 proyectos de `src/`, y en vivo contra el stack Docker (11 requests seguidos a `/api/auth/login` → los primeros 10 dan 401, el 11º da 429). **Límite conocido, fuera de alcance por decisión explícita** (ver README): el secreto JWT hardcodeado en `appsettings.Development.json`, el CORS hardcodeado a `localhost`, y la ausencia de security headers (HSTS/CSP/X-Frame-Options) — son gaps de configuración de despliegue, no de código, y quedan documentados como checklist de producción.
- [x] Auditoría de permisos por rol contra la matriz de §2.2 ✅ (2026-08-31) — comparé los permisos realmente sembrados en `DbSeeder.cs` y las policies `[Authorize(Policy="modulo.accion")]` de los 22 controllers contra la matriz completa. **Lo que coincide**: los permisos de SuperAdmin/Administrador/Empleado-Secretaría matchean la matriz exactamente (incluida la excepción de aprobar/rechazar solicitudes de membresía, exclusiva de Admin/SuperAdmin); la regla transversal de Ficha Médica funciona bien (`SociosController.PuedeVerFichaMedicaCompleta`: nivel jerárquico ≤2, resto de roles recibe los campos crudos en `null` y solo el semáforo de vigencia); las restricciones "Propio" de Reservas y Cuotas/Pagos del Socio están bien implementadas con filtro explícito por `SocioId` del JWT (no hay forma de que un Socio vea/pague la reserva o cuota de otro); el bug histórico de Etapa 6 (permisos declarados pero nunca concatenados al seed) no se repite — confirmado línea por línea.

  **Gaps encontrados — no son fallas de seguridad (nadie accede de más), son funcionalidad de la matriz que nunca se construyó:**
  - **Gestión de Usuarios Admin / Gestión de Roles-Permisos**: no existe ningún `UsuariosController`/`RolesController` ni permisos `usuarios.*`/`roles.*` en el catálogo — las 2 filas de la matriz están sin ningún backend.
  - **Autoservicio del Socio incompleto**: `MePortalController` (Etapa 2) cubre Reservas/Cuotas/Comunicaciones/Consultas con el patrón correcto, pero la matriz también le da al Socio: ver/editar su propia ficha de Socio (Propio L, M parcial), ver su Grupo Familiar (Propio L), listar Actividades (L) e inscribirse/darse de baja de una (Propio C/B) — ninguno de estos 4 tiene endpoint. Como consecuencia directa, Instructor y Socio no tienen ningún `RolPermiso` sembrado (por diseño, ya que sus accesos son todos "Propio"/fuera del sistema de permisos por rol), así que tampoco podrían usar los endpoints de staff existentes aunque quisieran.
  - **Catálogo público (No Socio)**: la matriz pide listar Actividades y Espacios sin login ("L público"); hoy solo `GET /api/configuracion/publica` y el alta de Solicitud de Membresía son anónimos, no hay variante pública de esos 2 catálogos.

  **Decisión de alcance (acordada con el usuario):** el SPEC.md pide una *auditoría*, no la construcción de las funcionalidades faltantes — cada gap de arriba es del tamaño de una pieza de trabajo propia (un módulo de Usuarios/Roles completo, 4 endpoints de autoservicio + su UI, un catálogo público) y se aborda por separado si se decide priorizarlo, no dentro de este ítem.
